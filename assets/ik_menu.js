;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype.init = function () {
		...
		if (!plugin.selected.length) {
			...
			plugin.selected
				.parentsUntil('nav', 'li').attr({'tabindex': 0});
	...
	Plugin.prototype.showSubmenu = function(event) {
		...
		$elem = $(event.currentTarget).attr({
            'aria-expanded': true,
            'tabindex': -1
        });
		$submenu = $elem.children('ul').attr({
            'aria-hidden': false
        });;
		
		if ($submenu.length) {
			$elem.addClass('expanded').attr({'aria-expanded':false});
