	...
	Plugin.prototype.init = function () {
		...
		if( !plugin.textfield.is(':text') ) {
			...
			plugin.knob = $('<div/>')
				.attr({
					'id': id,
					'tabindex': 0, //makes the slider keyboard focusable
					'role': 'slider',
					'aria-valuemax': plugin.options.minValue,
			...
			$('<div/>').attr({
				'id': id + '_instructions'
			}).text(this.options.instructions).addClass('ik_readersonly').appendTo(this.element);
